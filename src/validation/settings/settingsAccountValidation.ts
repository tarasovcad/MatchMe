...
export const settingsAccountValidationSchema = z
  .object({
    ...
    time_commitment: z
      ...
      .optional()
      .refine(
        (val) => !val || val === "" || timeCommitment.some((option) => option.value === val),
        {
    ...
    about_you: z
      ...
      .refine(
        ...
        {
          message:
            "About you section must contain at least 3 words to provide a meaningful description",
        },
      )
      .optional(),
    personal_website: z
  ...
  .refine(
    (data) => {
      // If profile is public, profile_image is required
      if (data.is_profile_public && (!data.profile_image || data.profile_image.length === 0)) {
        return false;
      }
      return true;
    },
    {
      message: "Profile image is required to make your profile public",
      path: ["profile_image"],
    },
  );
