...
const PROFILES_PAGE_KEY = "profiles";

const ProfilesClientComponent2 = ({userSession}: {userSession: User | null}) => {
  ...
  const fetchProfiles = async () => {
    ...
    const allProfiles = await getAllProfiles(1, 10);
    ...
    console.log(`Fetched profiles in ${endTime.getTime() - startTime.getTime()}ms`);
    setIsLoading(false);
  };
      ...
      <div className="flex flex-col gap-4">
        <div className="flex max-[480px]:flex-col justify-between items-center gap-3 max-[480px]:gap-2">
          ...
          <div className="flex gap-3 max-[480px]:gap-2 max-[480px]:w-full">
            ...
            <FilterButton pageKey={PROFILES_PAGE_KEY} data={profileFiltersData} />
          </div>
        </div>
        <FilterPanel pageKey={PROFILES_PAGE_KEY} />
...
export default ProfilesClientComponent2;
