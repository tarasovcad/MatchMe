...
const AuthSignUpClientPage = ({
  ...
  useEffect(() => {
    if (isProfileComplete !== null) {
      setCurrentStep(isProfileComplete ? 3 : 1);
    }
  }, [isProfileComplete]);
  ...
  const methods = useForm<SignUpFormData>({
    resolver: zodResolver(
      getSchemaForStep(currentStep === null ? 1 : currentStep),
    ),
    mode: "onChange",
    defaultValues: {
      email: "",
  ...
  const {
    ...
    bottomSubTitle,
  } = config[currentStep === null ? 1 : currentStep];
    ...
    <div className="min-h-screen flex flex-col px-4 py-8">
      ...
      <form
        ...
        onSubmit={methods.handleSubmit(
          handleFormSchema(currentStep === null ? 1 : currentStep),
        )}>
